...
public class AutoCreateObject : MonoBehaviour {
	...
	void Update () {
		...
		if (timer >= createTime && cnt < total) {	//当生成时间间隔大于等于生成时间时
	...
	void CreateObject(){	
        ...
        if (newGameObject.GetComponent<EthanTrace>() != null)   //设置敌人的追踪目标
            newGameObject.GetComponent<EthanTrace>().target = targetTrace;
    }
}
