...
public class GameManager : MonoBehaviour
{
    ...
    { Playing, GameOver, Winning, Pausing };
    ...
    void Update()
    {
        ...
        switch (gameState)
        {
            ...
            case GameState.Playing:
                if (Input.GetKeyDown(KeyCode.R))       //输入R键，控制鼠标光标的可见性
                    Cursor.visible = !Cursor.visible;
                if (Input.GetKeyDown(KeyCode.Escape))
                    gm.gameState = GameState.Pausing;
                if (Input.GetKeyDown(KeyCode.M)) {
                    isMap = !isMap;
                    BigMapCanvas.SetActive(isMap);
                }
                if (playerHealth.isAlive == false)          //若玩家死亡，游戏状态切换到游戏失败
                    gm.gameState = GameState.GameOver;
                else if (currentScore >= TargetScore)
                {       //若当前得分大于等于目标分数，游戏状态切换到游戏胜利
                ...
                {
                    scoreText.text = "Current Score ： " + currentScore; //显示当前游戏得分
                    ...
                    timeText.text = "Game Time ： " + currentTime.ToString("0.00");  //显示已用时间
            ...
            case GameState.Pausing:
                if (!isGameOver)
                {
                    ...
                    Cursor.visible = true;                  //将鼠标光标显示
                    playingCanvas.SetActive(false);     //禁用游戏进行中Canvas
                    gameResultCanvas.SetActive(false);       //启用游戏结果Canvas
                    if (mobileControlRigCanvas != null)     //禁用移动端控制Canvas
                        mobileControlRigCanvas.SetActive(false);
    ...
    void EditGameOverCanvas()
    {
        ...
        {
            case "1":
                gameMessage.text = "Congratulations you are the No.1 ！";
                break;
            case "2":
                gameMessage.text = "Congratulations you are the No.2 ！";
                break;
            case "3":
                gameMessage.text = "Congratulations you are the No.3 ！";
                break;
            default:
                gameMessage.text = "Oh you still need efforts !";
                break;
        }
    ...
    public void Continue()
    {
        ...
        Time.timeScale = 1;
        Cursor.visible = false;                  //将鼠标光标显示
        playingCanvas.SetActive(true);     //禁用游戏进行中Canvas
        gameResultCanvas.SetActive(false);       //启用游戏结果Canvas
        if (mobileControlRigCanvas != null)     //禁用移动端控制Canvas
            mobileControlRigCanvas.SetActive(false);
        pauseCanvas.SetActive(false);
